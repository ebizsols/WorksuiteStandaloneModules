...
class AddOwnedByAddedByLastUpdatedByColumnsExpenses extends Migration
{
    ...
    public function up()
    {
        ...
        if (!is_null($admin)) {
            Expense::whereNull('added_by')->update(['added_by' => $admin->id]);
            Expense::whereNull('last_updated_by')->update(['last_updated_by' => $admin->id]);

            ExpensesCategory::whereNull('added_by')->update(['added_by' => $admin->id]);
            ExpensesCategory::whereNull('last_updated_by')->update(['last_updated_by' => $admin->id]);
        }
    ...
    {
        Permission::where('module_id', 17)->where('is_custom', 1)->delete();
