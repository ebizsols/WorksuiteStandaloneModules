...
class AddViewEarningPermissionTimelogs extends Migration
{
    ...
    public function up()
    {
        ...
        $module = Module::where('module_name', 'timelogs')->first();
        $taskModule = Module::where('module_name', 'tasks')->first();

        $employeeCustomPermisisons = [
            'view_timelog_earnings'
        ];

        $taskCustomPermisisons = [
            'create_unassigned_tasks'
        ];
        ...
        foreach ($taskCustomPermisisons as $permission) {
            $perm = Permission::create([
                'name' => $permission,
                'display_name' => mb_ucwords(str_replace('_', ' ', $permission)),
                'is_custom' => 1,
    ...
    {
        Permission::where('name', 'view_timelog_earnings')->delete();
        Permission::where('name', 'create_unassigned_tasks')->delete();
    }
