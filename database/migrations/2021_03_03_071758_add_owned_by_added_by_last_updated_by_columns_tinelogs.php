...
class AddOwnedByAddedByLastUpdatedByColumnsTinelogs extends Migration
{
    ...
    public function up()
    {
        ...
        if (!is_null($admin)) {
            ProjectTimeLog::whereNull('added_by')->update(['added_by' => $admin->id]);
            ProjectTimeLog::whereNull('last_updated_by')->update(['last_updated_by' => $admin->id]);
        }
    ...
    {
        Permission::where('module_id', 9)->where('is_custom', 1)->delete();
